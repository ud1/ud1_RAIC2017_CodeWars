...
MyMove Strat::calcNextMove()
{
	...
	if (!distributionMatrixInitialized)
		distributionMatrixFOWBlured = distributionMatrix;
	...
	if (enableFOW && distributionMatrixInitialized)
	{
		...
		//spreadDistributionMatrix(distributionMatrixFOWBlured);
		...
		for (int y = 0; y < DISTR_MAT_CELLS_Y; ++y)
		{
			for (int x = 0; x < DISTR_MAT_CELLS_X; ++x)
			{
				...
				DistributionMatrix::Cell &simDcell = distributionMatrixFOWBlured.getCell(x, y);
				double visFactor = visibilityFactors[y * DISTR_MAT_CELLS_X + x];
				
				constexpr double rememberTicks = 100.0;
				if (visFactor > 0.5)
				{
					simDcell = dcell;
					simDcell.updateTick = tick;
					simDcell.realUpdateTick = tick;
				}
				else
				{
					double dt = tick - simDcell.realUpdateTick;
		...
		for (int i = 0; i < DISTR_MAT_CELLS_X * DISTR_MAT_CELLS_Y; ++i)
		{
			DistributionMatrix::Cell &simDcell = distributionMatrixFOWBlured.cells[i];
			simDcell.totalHealth = 0.0;
			for (int k = 0; k < 5; ++k)
				simDcell.totalHealth += simDcell.health[k];
		}
	}
	...
	if (result.action == MyActionType::NONE && !groups.empty())
	{
		...
		{
			/*for (int i = 0; i < DISTR_MAT_CELLS_X * DISTR_MAT_CELLS_Y; ++i)
				highPriorityCells[i] = false;*/
			
			for (Group &g : groups)
			{
				g.invisible = true;
				
				/*P targetPoint = g.center + g.shift * (150.0 * unitVel(g.unitType));
				for (int y = 0; y < DISTR_MAT_CELLS_Y; ++y)
				{
					for (int x = 0; x < DISTR_MAT_CELLS_X; ++x)
					{
						bool &val = highPriorityCells[y * DISTR_MAT_CELLS_X + x];
						if (!val)
						{
							P p = P((x + 0.5) * DISTR_MAT_CELL_SIZE, (y + 0.5) * DISTR_MAT_CELL_SIZE);
							if (p.dist2(targetPoint) < sqr(300.0))
							{
								val = true;
							}
						}
...
void Strat::spreadDistributionMatrix(DistributionMatrix &distrMatrix)
{
	double horTicks[5];
	double diagTicks[5];
	...
	for (int y = 0; y < DISTR_MAT_CELLS_Y; ++y)
	{
		for (int x = 0; x < DISTR_MAT_CELLS_X; ++x)
		{
			DistributionMatrix::Cell &dcell = distrMatrix.getCell(x, y);
			...
			for (int i = 0; i < 5; ++i)
			{
				for (int j = 0; j < 8; ++j)
				{
					...
					if (dcell.updateTick + dt < tick)
					{
						...
						if (x2 >= 0 && x2 < DISTR_MAT_CELLS_X && y2 >= 0 && y2 < DISTR_MAT_CELLS_Y)
						{
							DistributionMatrix::Cell &othCell = distrMatrix.getCell(x2, y2);
	...
	for (int y = 0; y < DISTR_MAT_CELLS_Y; ++y)
	{
		for (int x = 0; x < DISTR_MAT_CELLS_X; ++x)
		{
			...
			if (anyDamage || (spy && dCell.totalEnemyHealth > 0))
			{
				...
				if (enableFOW && pts < 0.0)
				{
					pts *= std::max(0.01, visFactor);
				}
		...
		if (res == 0.0)
		{
			for (int y = 0; y < DISTR_MAT_CELLS_Y; ++y)
			{
				for (int x = 0; x < DISTR_MAT_CELLS_X; ++x)
				{
					...
					if (visFactor <= 0.5)
					{
						DistributionMatrix::Cell &cell = distributionMatrixFOWBlured.getCell(x, y);
						...
						if (cell.totalHealth)
						{
							...
							double pts = cell.totalHealth * dt * (1.0 - visFactor) * 0.00001 / (1.0 + dist2);
							res += pts;
						}
					}
				}
...
bool Strat::canMove(const P &shift, const Group &group) const
{
	...
	bbox.p1 -= P(eps, eps);
	bbox.p2 += P(eps, eps);
	const double shiftDt = shift.len() / unitVel(group.unitType);
	
	for (const MyUnit &p : units)
	{
		if (!group.check(p) && group.canIntersectWith(p))
		{
			P relEnd = p.pos + p.vel * shiftDt - shift;
			bool intersectsBBox = bbox.intersectsSegment(p.pos, relEnd);
			if (intersectsBBox)
				return false;
		}
	}
...
bool Strat::canMoveDetailed(const P &shift, const Group &group, const std::vector<const MyUnit *> &groupUnits, const std::vector<const MyUnit *> &otherUnits) const
{
	if (!canMoveNuke(shift, group))
		return false;
	...
	for (const MyUnit *p : otherUnits)
	{
		...
		if (intersectsBBox)
		{
			...
			{
				P p2 = myp->pos + shift - p->vel * shiftDt;
